...
from interfaz.c_bloque_acciones_independientes.bloque_subsistemas import (
    mostrar_subsistemas_combobox,
)
from interfaz.c_bloque_acciones_independientes.bloque_proyectos import (
    mostrar_proyectos_combobox,
)
from interfaz.c_bloque_acciones_independientes.bloque_documentos import (
    mostrar_documentos_combobox,
)
...
def crear_bloque_consulta(frame_funcionalidades, traducciones, frame_visual):
    ...
    boton_consulta = tk.Button(
        frame_consulta,
        text=traducciones["P_DESCARGA"],
        command=lambda: descargar_csv(traducciones, frame_visual),
    )
    ...
    checkbox_requisitos = tk.Checkbutton(
        filtros_frame,
        text=traducciones["P_TICK_REQUISITOS"],
        variable=var_requisitos,
        command=lambda: seleccionar_unico(var_requisitos),
    )
    checkbox_requisitos.grid(row=0, column=0, padx=5, pady=5, sticky="ew")

    checkbox_documentos = tk.Checkbutton(
        filtros_frame,
        text=traducciones["P_TICK_DOCUMENTOS"],
        variable=var_documentos,
        command=lambda: seleccionar_unico(var_documentos),
    )
    checkbox_documentos.grid(row=0, column=1, padx=5, pady=5, sticky="ew")

    checkbox_proyectos = tk.Checkbutton(
        filtros_frame,
        text=traducciones["P_TICK_PROYECTOS"],
        variable=var_proyectos,
        command=lambda: seleccionar_unico(var_proyectos),
    )
    checkbox_proyectos.grid(row=0, column=2, padx=5, pady=5, sticky="ew")

    checkbox_subsistemas = tk.Checkbutton(
        filtros_frame,
        text=traducciones["P_TICK_SUBSISTEMAS"],
        variable=var_subsistemas,
    ...
    label_subsistemas = tk.Label(
        frame_consulta, text=traducciones["P_FILTRO_SUBSISTEMAS"], bg="white"
    )
    label_subsistemas.grid(row=2, column=0, padx=10, pady=5, sticky="w")

    combobox_subsistemas = ttk.Combobox(
        frame_consulta,
        state="readonly",
        postcommand=lambda: mostrar_subsistemas_combobox(
            traducciones, combobox_subsistemas
        ),
    )
    combobox_subsistemas.grid(row=3, column=0, padx=10, pady=5, sticky="ew")

    # Filtro de Proyectos dentro del bloque 2
    label_proyectos = tk.Label(
        frame_consulta, text=traducciones["P_FILTRO_PROYECTOS"], bg="white"
    )
    label_proyectos.grid(row=4, column=0, padx=10, pady=5, sticky="w")

    combobox_proyectos = ttk.Combobox(
        frame_consulta,
        state="readonly",
        postcommand=lambda: mostrar_proyectos_combobox(
            traducciones, combobox_proyectos
        ),
    )
    combobox_proyectos.grid(row=5, column=0, padx=10, pady=5, sticky="ew")

    # Filtro de Documentos dentro del bloque 2
    label_documentos = tk.Label(
        frame_consulta, text=traducciones["P_FILTRO_DOCUMENTOS"], bg="white"
    )
    label_documentos.grid(row=6, column=0, padx=10, pady=5, sticky="w")

    combobox_documentos = ttk.Combobox(
        frame_consulta,
        state="readonly",
        postcommand=lambda: mostrar_documentos_combobox(
            traducciones, combobox_documentos
        ),
    )
