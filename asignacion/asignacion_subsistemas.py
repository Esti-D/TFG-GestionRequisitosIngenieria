...
Descripción: Funciones para asignar subsistemas a un documento en función de palabras
clave mediante estructuras Trie.
...
import os
from asignacion.cargar_palabras import cargar_palabras_desde_csv
from asignacion.trie import Trie
...
def construir_trie_para_subsistemas(subsistemas_palabras):
    ...
    Args:
        subsistemas_palabras (dict): Diccionario con los subsistemas como claves y listas
        de palabras clave como valores.

    Returns:
        dict: Diccionario donde las claves son los subsistemas y los valores son objetos Trie
        construidos con las palabras clave.
    """
    subsistemas_trie = {}
...
def asignar_subsistemas_a_documento_trie(texto_documento, subsistemas_trie):
    ...
    for subsistema, trie in subsistemas_trie.items():
        contador_coincidencias = sum(
            1 for palabra in palabras_documento if trie.buscar_palabra(palabra)
        )

        if (
            contador_coincidencias >= 3
        ):  # Sugerir subsistema si hay al menos 3 coincidencias
...
def asignar_subsistemas_a_documento(texto_documento):
    ...
    subsistemas_trie = construir_trie_para_subsistemas(
